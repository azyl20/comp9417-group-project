...
labels = ['stack_train', 'forest_train',
          'boost_train', 'svm_train', 'log_train']

i = 0
for data in [stack_train, forest_train, boost_train, svm_train, log_train]:
...
labels = ['stack_test', 'forest_test',
          'boost_test', 'svm_test', 'log_test']
i = 0
for data in [stack_test, forest_test, boost_test, svm_test, log_test]:
    plt.scatter(list(range(200)), data, label=labels[i], marker='.')
    i += 1
...
print(f"log test average: {sum(log_test)/len(log_test)}")
...
axs[0, 2].boxplot(boost_test)
axs[0, 2].set_title('HistGBC scores')
axs[1, 0].boxplot(svm_test)
axs[1, 0].set_title('SVM scores')
axs[1, 1].boxplot(log_test)
axs[1, 1].set_title('log scores')

for i in range(2):
    for j in range(3):
        axs[i, j].set_ylim(0.75, 1)
